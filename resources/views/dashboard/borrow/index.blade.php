        ...
        <div id="Add" class="bg-white dark:bg-gray-800/50 dark:border dark:border-gray-700/50 text-gray-900 dark:text-white/80 mb-3 p-5 rounded shadow-md hidden">
            <form action="{{ route('borrows.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 grid-cols-2 sm:gap-3">
                  <div>
                      <label for="item_id" class="block mb-2 text-sm font-medium">Barang Dipinjam</label>
                      <select id="item_id" name="item_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-sm rounded-lg block w-full p-2.5">
                  ...
                  <div>
                      <label for="quantity" class="block mb-2 text-sm font-medium">Jumlah Dipinjam</label>
                      <input type="number" name="quantity" id="quantity" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-sm rounded-lg block w-full p-2.5" placeholder="Stok Barang" required>
                  </div>
                  <div>
                      <label for="borrow_date" class="block mb-2 text-sm font-medium">
                          Tanggal Pinjam
                      </label>
                      <input 
                          type="datetime-local" 
                          ...
                          class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-sm rounded-lg block w-full p-2.5"
                          required
                      >
                  </div>
                  <div>
                      <label for="location_id" class="block mb-2 text-sm font-medium">Lokasi</label>
                      <select id="location_id" name="location_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-sm rounded-lg block w-full p-2.5">
            ...
            <form action="{{ route('borrows.update', $edit->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 grid-cols-2 sm:gap-3">
                    <div>
                        ...
                        <input type="text" value="{{ $edit->user?->name }}" disabled id="user_id" name="user_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-gray-900 dark:text-white text-sm rounded-lg block w-full p-2.5">
                    </div>
                    <div>
                        <label for="item_id" class="block mb-2 text-sm font-medium">Barang Dipinjam</label>
                        <input type="text" value="{{ $edit->item?->name }}" disabled id="item_id" name="item_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-gray-900 dark:text-white text-sm rounded-lg block w-full p-2.5">
                    </div>
                    <div>
                        <label for="quantity" class="block mb-2 text-sm font-medium">Jumlah Dipinjam</label>
                        <input type="number" value="{{ $edit->quantity }}" name="quantity" id="quantity" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-gray-900 dark:text-white text-sm rounded-lg block w-full p-2.5" required>
                    </div>

                    <div>
                        <label for="location_id" class="block mb-2 text-sm font-medium">Lokasi</label>
                        <select id="location_id" name="location_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-gray-900 dark:text-white text-sm rounded-lg block w-full p-2.5">
                    ...
                    <div class="col-span-full">
                        ...
                        <input 
                            ...
                            class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700/50 text-gray-900 dark:text-white text-sm rounded-lg block w-full p-2.5"
                            required
                        >
                    </div>
                </div>
                <button type="submit" class="px-5 py-2.5 bg-teal-700 text-white rounded-lg hover:bg-teal-800 mt-4 text-sm font-medium">Update</button>
                <a href="{{ route('borrows.index') }}" class="px-4 py-2.5 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-medium text-sm">Cancel</a>
            </form>
        </div>
        @endif
        {{-- EDIT FORM --}}
          ...
          <div class="-m-1.5 overflow-x-auto">
              <div class="p-1.5 min-w-full inline-block align-middle">
                  <div class="overflow-hidden">
                          ...
                          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                              @forelse ($borrows->whereIn('status', ['pending', 'ongoing', 'done'])->sortBy(fn($b) => array_search($b->status, ['pending', 'ongoing', 'done'])) as $data)
                                  <tr>
                                      ...
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $data->user?->name ?? '-' }}</td>
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $data->item?->name ?? '-' }}</td>
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $data->location?->name ?? '-' }}</td>
                                      ...
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $data->quantity }}</td>
                                      @php
                                          $statusClass = match($data->status) { 
                                              'done' => 'bg-emerald-100 text-emerald-500 dark:bg-emerald-900/40 dark:text-emerald-300',
                                              'ongoing' => 'bg-sky-100 text-sky-500 dark:bg-sky-900/40 dark:text-sky-300',
                                              default => 'bg-orange-100 text-orange-500 dark:bg-orange-900/40 dark:text-orange-300',
                                          };
                                      @endphp
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                      ...
                      @if($borrows->total() > $n)
                      <div class="px-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                          {{ $borrows->links() }}
                      </div>
                      @endif
                  </div>
