<x-app-layout>
    ...
    <div class="px-12 py-4 mt-4">
        <div class="w-3/4">
        <form acti  on="{{ route('borrows.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="grid gap-4 grid-cols-2 sm:gap-3">
            <div>
              ...
              <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" placeholder="Nama Barang" required value="{{ $borrows->user?->name ?? '-' }}" disabled>
            </div>
            <div>
